...
(use-package spaceink-theme
  :ensure (:host github :repo "bcardoso/spaceink-theme")
  :config
...
(use-package evil
  ...
  ;TODO: check if the frame is already open if so open a new buffer
  ;with documentation, calling `eldoc-display-in-buffer'
