...
Only install these packages for ~x86_64~ architecture, ~tree-sitter~ binaries for arm are not available yet.
...
Fix error with ~TAB~ in ~evil-mode~ in ~org-mode~ with headings.

#+begin_src emacs-lisp
(defun my/org-tab ()
  "Run `org-cycle' only at point of a heading."
  (interactive)
  (if (org-at-heading-p)
      (org-cycle)
    (evil-jump-forward)))
