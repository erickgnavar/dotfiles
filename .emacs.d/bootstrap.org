...
Setup ligatures using ~Fira-Code-Symbol~, install it from [[https://github.com/jming422/fira-code-mode/blob/master/fonts/FiraCode-Regular-Symbol.otf][GitHub]].
...
(use-package eglot
  ...
  :config
  (setf (alist-get 'elixir-ts-mode eglot-server-programs) `(,(expand-file-name "~/Code/oss/elixir-ls/release/language_server.sh"))))
#+end_src
...
((elixir-ts-mode . ((eglot-workspace-configuration . ((:elixirLS . (:projectDir "mix.exs dir/")))))))
#+end_src
...
(use-package elixir-ts-mode
  ...
  :config
  ;; for some reason there is not map already defined
  (evil-define-key nil elixir-ts-mode-map (kbd "<leader>d") 'my/goto-definition-dumb-jump-fallback))
#+END_SRC
...
For some reason using ~:mode-hydra~ inside ~use-package~ doesn't load environment variables so we can run mix commands properly,
that's why we need to define this ~hydra~ this way.

#+begin_src emacs-lisp
(with-eval-after-load 'major-mode-hydra
  (major-mode-hydra-define elixir-ts-mode
...
(use-package typescript-ts-mode
  ...
  :mode "\\.tsx?\\'"
  :config
  (evil-define-key nil typescript-ts-mode-map (kbd "<leader>d") 'my/goto-definition-dumb-jump-fallback))
#+end_src
