...
;; we tell straight.el that project is a builtin package so it won't be downloaded
(straight-use-package '(project :type built-in))

(with-eval-after-load 'project
  (define-key project-prefix-map (kbd "D") 'my/project-edit-direnv)
  (define-key project-prefix-map (kbd "d") 'project-dired)
  (define-key project-prefix-map (kbd "e") 'my/project-edit-dir-locals)
  (define-key project-prefix-map (kbd "k") 'my/project-kill-buffers)
  (define-key project-prefix-map (kbd "n") 'my/project-open-new-project)
  (define-key project-prefix-map (kbd "p") 'my/project-switch))
#+end_src
...
#+begin_src emacs-lisp
;; we tell straight.el that eglot is a builtin package so it won't be downloaded
(straight-use-package '(eglot :type built-in))

(with-eval-after-load 'eglot
  (define-key eglot-mode-map (kbd "C-c C-d") 'eldoc-doc-buffer)
  (define-key eglot-mode-map (kbd "C-c C-s") 'xref-find-references)
...
(defun my/goto-definition-dumb-jump-fallback ()
  ...
  (interactive)
  (if (and (fboundp 'eglot) (eglot-managed-p))
