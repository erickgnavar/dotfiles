...
(use-package evil-leader
  ...
  (evil-leader/set-key
    ...
    "A" 'my/grep-in-project
    "ba" 'my/add-bookmark
    "bb" 'my/bookmark-switch
    "B" '(lambda ()
           (interactive)
...
#+begin_src emacs-lisp
(defun my/after-embark-export ()
  "Enable `wgrep-mode' only for exported grep results."
  (when (cl-search "Exported grep results" (buffer-string))
...
(use-package embark
  ...
  ;; we need to setup this hook because `embark-export' will
  ;; run it after the new buffer(with the exported data) is created
  (add-hook 'embark-after-export-hook 'my/after-embark-export))
...
;TODO: fix order in results
(defun my/git-backup ()
  "Navigate in verisons of the current file."
  (interactive)
  (unless (featurep 'git-backup)
    (require 'git-backup))
...
(use-package rust-mode
  ...
  :config
  (evil-leader/set-key-for-mode 'rust-mode "d" 'my/goto-definition-dumb-jump-fallback))
...
(defun my/bookmark-switch ()
  ...
  (interactive)
  (let* ((options (mapcar 'car my/bookmarks))
...
(defun my/add-bookmark ()
  ...
  (interactive)
  (setf (alist-get (buffer-name) my/bookmarks ) (current-buffer))
  (message "%s added to bookmarks" (buffer-name)))
#+end_src
